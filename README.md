...
`maestro validate <issue> --retry` is the narrower recovery path for a validator infrastructure or transient execution failure. It atomically resolves the issue's current implementation, verifies the persisted branch, worktree, base and exact HEAD are still clean and that the acceptance/policy digest is unchanged, then creates a validation-only child run against that same commit. It never launches a worker or charges a correction attempt. The failed validator remains in the parent evidence, while the fresh result becomes authoritative through normal lineage resolution. Semantic `REWORK`, `HUMAN_GATE`, oversized authoritative reports, superseded implementations, active operations, completed work, and Git/context drift fail closed.

`maestro resolve <issue> --agent` is the issue-oriented front door for a current managed refresh conflict; it routes integration conflicts through fresh reconciliation validation and REWORK refresh conflicts back into their already-charged correction. To hand Maestro a merge or rebase that you started yourself, run `maestro resolve --adopt --agent` from that conflicted checkout. Adoption records the current index, partial resolutions, staged and unstaged patches, and recoverable untracked-file contents outside the checkout instead of aborting or recreating the operation; exact unrelated contents are checked again after resolution and checks. Configure `resolution.commands` (or existing `integration.commands`) so standalone adoption has explicit checks; without them Maestro refuses to claim validation. Each attempt is durably charged while atomically reserving shared repository capacity before the resolver starts, and every terminal path releases that slot. Interrupted managed and adopted resolvers retain their charge and original deadline when the same run resumes. Passing checks must leave the exact verified branch, HEAD, and preserved user state unchanged. An issue association is rejected when any current lifecycle owner or human gate already exists. A stopped adoption resumes idempotently with `maestro resolve --continue --agent`. Adoption authorizes only the current operation and checks—never push, default-branch integration, issue closure, unrelated edits, or another worker.
...
When refresh succeeds but a pre-merge integration command fails, Maestro records the exact command/output and starts a capacity-reserved correction on the refreshed worker branch. Correction attempts share the configured `resolution.maxAttempts` budget (default three) and one persisted deadline across restarts; an interrupted running attempt remains charged and the same correction run resumes. Each result receives a fresh validator, which must leave the verified branch and HEAD unchanged, and success returns to human review with no inherited approval. It does not blindly retry integration or merge/push the corrected result.
...
`maestro details <issue...>` is the verbose issue-level evidence view. It resolves each issue's latest relevant persisted run independently and shows worker results, commits, validator verdicts and reports, human review, integration state, and branch/worktree debugging identifiers. Rework, reconciliation, and validator-retry children also include their parent/source evidence, including retry reason and diagnostic truncation metadata. Use `--run <run-id>` to inspect an explicit historical run; no workers or validators are rerun.

`maestro status` is the concise decision surface between workflow actions. Its default view groups action-first lifecycle state, shows the effective scheduler's exact next selection in planner order, bounds issue previews, run-readiness diagnostics, and recommendation actions with exact hidden counts, and collapses completed history to a count. Displayed commit-ready integration sets retain their exact executable command. It keeps validator verdicts distinct from human dispositions and ends with state-derived next commands. Use `maestro status --all` to expand the full effective issue inventory, run-readiness diagnostics, and actions, `maestro status --completed` for completed history, or `maestro status <issue...>` for a focused view with the current worker commit, validator verdict, human-review state, integration eligibility, and next actions. Expansion flags cannot be combined with focused issue numbers. `--watch` uses the same grouping and filtering rules. `maestro plan` remains available as a manifest-only planning/debugging preview, but is not required in the routine loop.
