...
`maestro resolve <issue> --agent` is the issue-oriented front door for a current managed refresh conflict; it routes integration conflicts through fresh reconciliation validation and REWORK refresh conflicts back into their already-charged correction. To hand Maestro a merge or rebase that you started yourself, run `maestro resolve --adopt --agent` from that conflicted checkout. Adoption preserves the current index, partial resolutions, unstaged edits, and untracked-file evidence instead of aborting or recreating the operation. Configure `resolution.commands` (or existing `integration.commands`) so standalone adoption has explicit checks; without them Maestro refuses to claim validation. A stopped adoption resumes idempotently with `maestro resolve --continue --agent`. Adoption authorizes only the current operation and checks—never push, default-branch integration, issue closure, unrelated edits, or another worker.
