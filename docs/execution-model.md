...
Status is a read-only projection over that current-state overlay and the effective manifest plan. The default view groups attention, human approval, integration readiness, active work, the exact scheduler selection, remaining ready/blocked work, and completed counts in operational order. Routine groups use bounded previews with exact hidden counts; `--all` expands one effective row per known issue and `--completed` expands terminal history without replaying run generations. Focused issue selection remains available independently. Status presents validator outcome, human disposition, and integration eligibility as separate dimensions. Recorded integration and reconciled external completion are terminal across run generations, so older approved workers cannot reappear as commit candidates. A manifest-complete item with execution history but neither kind of completion evidence is instead an explicit consistency conflict and remains visible under attention; status does not invent evidence or recommend approval, rework, or commit for it. Commit readiness is evaluated against every item in each relevant persisted run, including source-run items superseded by a child correction, so a newer child cannot hide a disposition still required to integrate an approved sibling. Each proposed integration set carries its executable current or source-qualified command. That source obligation disappears when no current sibling remains eligible for integration, preventing settled history from becoming a phantom blocker. Focused status adds the latest worker commit and explicit evidence fields without becoming a replacement for verbose `details` output.
