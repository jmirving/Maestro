...
Correction admission also compares the expected source run and actionable rework state under that same lock. If another invocation approves, gates, integrates, or otherwise supersedes the discovered evidence before reservation, the stale task is discarded without starting a worker or consuming capacity. A prepared reservation that reaches a terminal pre-execution outcome is persisted as released and excluded from current workflow evidence.
