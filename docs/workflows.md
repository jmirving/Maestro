...
Standalone adoption requires `resolution.commands` in `.maestro.json`, or falls back to `integration.commands`. It snapshots the active operation, branch, index, staged/unstaged diffs, conflict paths, and content-hashed recoverable bytes for dirty and untracked files into the reported out-of-checkout recovery artifact before the agent runs. Existing partial resolutions are not reset. Each resolver attempt is persisted and reserves one repository-wide worker slot before invocation; the slot stays reserved through configured checks and is released on success, failure, timeout, exhaustion, or failed validation. An optional issue association is accepted only when that issue has no current run ownership or human gate. Only merge and rebase are supported; cherry-pick, revert, non-conflict Git failures, semantic ambiguity, and changed unrelated contents stop with actionable evidence. Passing checks validate the adopted result but do not grant approval, push, integrate, close an issue, or continue another lifecycle.
